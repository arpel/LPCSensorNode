...
#elif defined(__AVR_DONT_CARE__) //Arduino Leonardo
...
void rf12_spiInit () {
	...
	ConfigStruct.ClkDiv = Chip_SPI_CalClkRateDivider(LPC_SPI, 2000000);
	...
	Chip_GPIO_SetPinDIRInput(LPC_GPIO_PORT, 0, RFM_nIRQ_PIN);
	//Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0, RFM_nIRQ_PIN, true);

	/* Empty */
	Chip_PININT_Init(LPC_PININT);

	/* Configure interrupt channel for the GPIO pin in SysCon block */
	Chip_SYSCTL_SetPinInterrupt(0, RFM_nIRQ_PIN);

	Chip_PININT_ClearIntStatus(LPC_PININT, PININTCH(0));
	Chip_PININT_ClearFallStates(LPC_PININT, PININTCH(0));
...
static uint8_t rf12_byte (uint8_t out) {
	...
	XfSetup.DataSize = 8;
	XfSetup.pRx = RxBuf;
	return Chip_SPI_RWFrames_Blocking(LPC_SPI, &XfSetup);
}
...
static uint16_t rf12_xferSlow (uint16_t cmd) {
	...
	XfSetup.DataSize = 16;
	XfSetup.pRx = RxBuf;
	uint16_t reply = Chip_SPI_RWFrames_Blocking(LPC_SPI, &XfSetup);

	//reply = (RxBuf[0] << 8) | (RxBuf[1] & 0x00FF);
	reply = RxBuf[0];
	return reply;
}
...
void rf12_sendStart_full (uint8_t hdr, const void* ptr, uint8_t len) {
    rf12_len = len;
...
void rf12_sendNow (uint8_t hdr, const void* ptr, uint8_t len) {
  ...
  rf12_sendStart_full(hdr, ptr, len);
}
...
void rf12_sendWait (uint8_t mode) {
    ...
    while (rxstate != TXIDLE)
        if (mode) {
            ...
            //if (pinStatescount < 128)
            //	pinStates[pinStatescount++] = Chip_GPIO_GetPinState(LPC_GPIO_PORT, 0, RFM_nIRQ_PIN);
        }
}
...
// caller should validate!    if (frequency < 96) frequency = 1600;
    ...
    dummy = rf12_xfer(0x0000); // initial SPI transfer added to avoid power-up problem

    pinStates[pinStatescount++] = Chip_GPIO_GetPinState(LPC_GPIO_PORT, 0, RFM_nIRQ_PIN);

    dummy = rf12_xfer(RF_SLEEP_MODE); // DC (disable clk pin), enable lbd

    // wait until RFM12B is out of power-up reset, this takes several *seconds*
    dummy = rf12_xfer(RF_TXREG_WRITE); // in case we're still in OOK mode
    //while (digitalRead(RFM_nIRQ_PIN) == 0)
    //    rf12_xfer(0x0000);

    while (Chip_GPIO_GetPinState(LPC_GPIO_PORT, 0, RFM_nIRQ_PIN) == false)
    	rf12_xfer(0x0000);

    dummy = rf12_xfer(0x80C7 | (band << 4)); // EL (ena TX), EF (ena RX FIFO), 12.0pF
    dummy = rf12_xfer(0xA000 + frequency); // 96-3960 freq range of values within band
...
uint8_t rf12_configSilent () {
    ...
    uint8_t i;
    uint8_t e;
    for (i = 0; i < RF12_EEPROM_SIZE; ++i) {
        e = eeprom_read_byte(RF12_EEPROM_ADDR + i);
                ...
                ezNextSend[1] = now +
                    (ezInterval > 0 ? 1000L * ezInterval
                                    : (nodeid >> 6) == RF12_868MHZ ?
                                            13 * (ezSendLen + 10) : 100);
            rf12_sendStart_full(RF12_HDR_ACK, ezSendBuf, ezSendLen);
            --ezPending;
        }
    }
